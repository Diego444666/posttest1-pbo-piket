...
public class LokasiManager {
    ...
    public void cari(Scanner sc) {
        ...
        for (LokasiPiket l : daftarLokasi) {
            if (l.getIdLokasi().toLowerCase().contains(kata) || l.getNamaLokasi().toLowerCase().contains(kata)) {
                System.out.println(l.toString()); ada = true;
            }
        }
        System.out.println(garis);
    ...
    public void update(Scanner sc) {
        ...
        System.out.print("Nama baru      : "); String nama = sc.nextLine().trim(); if (!nama.isEmpty()) target.setNamaLokasi(nama);
        System.out.print("Deskripsi baru : "); String desk = sc.nextLine().trim(); if (!desk.isEmpty()) target.setDeskripsi(desk);
        System.out.print("Lantai baru    : "); String lantai = sc.nextLine().trim(); if (!lantai.isEmpty()) target.setLantai(lantai);
        System.out.print("Kapasitas baru : ");
        String kap = sc.nextLine().trim();
        if (!kap.isEmpty()) { try { target.setKapasitasOrang(Integer.parseInt(kap)); } catch (NumberFormatException e) { BaseManager.cetakError("Tidak valid."); } }
    ...
    public void hapus(Scanner sc) {
        ...
        if (sc.nextLine().trim().equalsIgnoreCase("y")) { daftarLokasi.remove(target); BaseManager.cetakOK("Berhasil dihapus!"); }
