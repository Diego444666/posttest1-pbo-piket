...
public class JadwalPiket {
    ...
    public void setHari(String hari) {
        for (String h : HARI_VALID) {
            if (h.equalsIgnoreCase(hari)) { this.hari = h; return; }
        }
        this.hari = "Senin";
    }

    public void setJamMulai(String jamMulai) {
        this.jamMulai = (jamMulai != null && jamMulai.matches("\\d{2}:\\d{2}")) ? jamMulai : "07:00";
    }

    public void setJamSelesai(String jamSelesai) {
        this.jamSelesai = (jamSelesai != null && jamSelesai.matches("\\d{2}:\\d{2}")) ? jamSelesai : "08:00";
    }

    public void setStatus(String status) {
        for (String s : STATUS_VALID) {
            if (s.equalsIgnoreCase(status)) { this.status = s; return; }
        }
