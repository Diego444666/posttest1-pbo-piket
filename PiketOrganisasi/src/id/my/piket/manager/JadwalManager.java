...
public class JadwalManager {
    ...
    public void tambah(Scanner sc) {
        ...
        if (anggota == null) { System.out.println("[!] Anggota tidak ditemukan."); return; }
        if (!anggota.isAktif()) { System.out.println("[!] Anggota ini tidak aktif."); return; }
        ...
        if (lokasi == null) { System.out.println("[!] Lokasi tidak ditemukan."); return; }
        ...
        daftarJadwal.add(new JadwalPiket(id, anggota.getIdAnggota(), anggota.getNama(),
                lokasi.getIdLokasi(), lokasi.getNamaLokasi(), hari, jamMulai, jamSelesai, "Terjadwal", ket));
        System.out.println("[OK] Jadwal ditambahkan! ID: " + id);
    }

    // ======= READ =======
    public void tampilSemua() {
        System.out.println("\n=== DAFTAR JADWAL PIKET ===");
        if (daftarJadwal.isEmpty()) { System.out.println("[!] Belum ada jadwal."); return; }
    ...
    public void update(Scanner sc) {
        ...
        System.out.println("  1. Ganti Anggota");
        System.out.println("  2. Ganti Lokasi");
        System.out.println("  3. Ganti Hari");
        System.out.println("  4. Ganti Jam");
        System.out.println("  5. Update Status");
        System.out.println("  6. Update Keterangan");
        System.out.print("Pilih: ");
        ...
        switch (sc.nextLine().trim()) {
            case "1":
                ...
                if (a != null) { target.setIdAnggota(a.getIdAnggota()); target.setNamaAnggota(a.getNama()); System.out.println("[OK] Anggota diupdate!"); }
            ...
            case "2":
                ...
                if (l != null) { target.setIdLokasi(l.getIdLokasi()); target.setNamaLokasi(l.getNamaLokasi()); System.out.println("[OK] Lokasi diupdate!"); }
                else System.out.println("[!] Tidak ditemukan.");
                break;
            case "3":
                for (int i = 0; i < HARI.length; i++) System.out.println("  " + (i+1) + ". " + HARI[i]);
                ...
                try {
                    int p = Integer.parseInt(sc.nextLine().trim()) - 1;
                    if (p >= 0 && p < HARI.length) { target.setHari(HARI[p]); System.out.println("[OK] Hari diupdate!"); }
                } catch (NumberFormatException e) { System.out.println("[!] Tidak valid."); }
                break;
            case "4":
                System.out.print("Jam Mulai baru  : ");
                String jm = sc.nextLine().trim();
                if (!jm.isEmpty()) target.setJamMulai(jm);
                System.out.print("Jam Selesai baru: ");
                String js = sc.nextLine().trim();
                if (!js.isEmpty()) target.setJamSelesai(js);
                System.out.println("[OK] Jam diupdate!");
                break;
            case "5":
                ...
                try {
                    int p = Integer.parseInt(sc.nextLine().trim()) - 1;
                    if (p >= 0 && p < STATUS.length) { target.setStatus(STATUS[p]); System.out.println("[OK] Status diupdate!"); }
                } catch (NumberFormatException e) { System.out.println("[!] Tidak valid."); }
                break;
            case "6":
                System.out.print("Keterangan baru: ");
                String ket = sc.nextLine().trim();
                if (!ket.isEmpty()) { target.setKeterangan(ket); System.out.println("[OK] Keterangan diupdate!"); }
                break;
            default:
